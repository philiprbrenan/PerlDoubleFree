...
#             +--        +---
        ...
        if ($i > 2 and j+2 < $fibers[i]->$#*)                                   # Try diagonally - this does not seem to occur much so we do not test all diaginal elemengs which would be expensive
         {                      my $A1 = v(i-1, j);                             # Above center
                                my $R2 = v(i+0, j+2);                           # Right bottom
                                my $L1 = v(i-2, j+1);                           # Lower center
                                my $L2 = v(i-2, j+2);                           # Lower left
          my $c1 = h(i-1, j+1); my $C1 = v(i-1, j+1);                           # Center
          my $r1 = h(i+0, j+1);                                                 # Right center
          my $a2 = h(i-2, j);                                                   # Above left
          my $b1 = h(i-1, j+2);                                                 # Below center
          my $b2 = h(i-2, j+2);                                                 # Below left

          if (
             !defined($A1) || $A1 ne a and                                      # Look for an opposite corner
              defined($a2) && $a2 eq a and                                      # Look for an opposite corner
              defined($R2) && $R2 eq a and
             !defined($R1) || $R2 ne a and
             !defined($c1) || $c1 ne a and
             !defined($C1) || $C1 ne a and
             !defined($L1) || $L1 eq a and
             !defined($L2) || $L2 eq a and
             !defined($b2) || $b2 eq a and
             !defined($b1) || $b1 eq a)
